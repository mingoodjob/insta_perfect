...
def profiles(uid):
    ...
    return render_template('profile.html', all_feed=all_feed, username=uid, write_count=write_count, userid=userid,
                           user_photo=user_photo, name=name, hobby=hobby, profile_desc=profile_desc)
...
def feed_number():
    ...
    if request.method == 'POST':
        ...
        if len(like_list) < 0:
            ...
            like = 0
        return jsonify(
            {'result': 'success', 'photo': photo, 'content': content, 'username': uid, 'like_count': like_count,
             'write_id': write_id, 'like': like, 'feed_number': feed_number, 'uid': uid})
